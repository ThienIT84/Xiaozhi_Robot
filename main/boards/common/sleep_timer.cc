...
void SleepTimer::CheckTimer() {
    ...
    if (seconds_to_light_sleep_ != -1 && ticks_ >= seconds_to_light_sleep_) {
        if (!in_light_sleep_mode_) {
            ...
            if (is_wake_word_running && !wake_word_in_sleep_enabled_) {
                audio_service.EnableWakeWordDetection(false);
                vTaskDelay(pdMS_TO_TICKS(100));
            }

            if (wake_word_in_sleep_enabled_ && is_wake_word_running) {
                ESP_LOGI(TAG, "Wake-by-voice enabled, keep CPU active in sleep mode");
            } else {
