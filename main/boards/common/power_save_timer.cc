...
void PowerSaveTimer::SetEnabled(bool enabled) {
    if (enabled && !enabled_) {
        ...
        wake_word_in_sleep_enabled_ = settings.GetBool("wake_word_in_sleep", true);
...
void PowerSaveTimer::PowerSaveCheck() {
    ...
    if (seconds_to_sleep_ != -1 && ticks_ >= seconds_to_sleep_) {
        if (!in_sleep_mode_) {
            ...
            if (cpu_max_freq_ != -1) {
                // Disable wake word detection only when wake-by-voice is disabled.
                auto& audio_service = app.GetAudioService();
                is_wake_word_running_ = audio_service.IsWakeWordRunning();
                if (is_wake_word_running_ && !wake_word_in_sleep_enabled_) {
                    audio_service.EnableWakeWordDetection(false);
                    vTaskDelay(pdMS_TO_TICKS(100));
                }
                // Keep audio input enabled for wake-by-voice.
                auto codec = Board::GetInstance().GetAudioCodec();
                if (codec && !wake_word_in_sleep_enabled_) {
